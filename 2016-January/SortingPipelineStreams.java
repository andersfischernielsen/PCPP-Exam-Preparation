...
class SortingPipelineStreams {
    public static void main(String[] args) {
        ...
        var l = DoubleStream.concat(input, flushing)
            .flatMap(d -> (DoubleStream) stage1.apply(d))
            .flatMap(d -> (DoubleStream) stage2.apply(d))
            .flatMap(d -> (DoubleStream) stage3.apply(d));
    ...
    private static DoubleFunction getStage(int maxSize, PriorityQueue<Double> heap) {
        return (n -> {
            heap.add(n);
            if (heap.size() > maxSize) return DoubleStream.of(heap.poll());
